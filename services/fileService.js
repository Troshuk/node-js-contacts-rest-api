...
const { TEMP_FOLDER, STATIC_FOLDER } = serverConfigs.FILE;

const MEGABYTE = 1024 * 1024;
const destination = path.resolve(TEMP_FOLDER);
const storagePath = path.resolve(STATIC_FOLDER);
...
const moveFileToPublic = (file, folder) =>
  fs.rename(file.path, path.join(storagePath, folder, file.filename));
